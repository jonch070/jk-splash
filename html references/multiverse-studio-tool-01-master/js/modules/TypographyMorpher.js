...
class TypographyMorpher {
    ...
    drawLayerSimple(ctx, layer, useInternalBW = false) {
        ...
        // Calculate text box width for wrapping
        const textBoxWidth = layer.textBoxWidth ? (width * layer.textBoxWidth / 100) : width * 0.8;
        const textAlign = layer.textAlign || 'center';

        // Wrap text into lines based on textBoxWidth
        const wrappedLines = this.wrapText(ctx, textString, textBoxWidth, layer);
        ...
        wrappedLines.forEach((line, i) => {
            ...
            // Calculate x offset based on alignment - ANCHOR-BASED
            // The aligned edge touches the origin (0,0)
            let xOffset = 0;
            ...
            if (textAlign === 'left') {
                ...
                ctx.drawingContext.textAlign = 'right';
                xOffset = 0;
            } else if (textAlign === 'justify') {
                // Justify: treat like left but spread words
                ctx.drawingContext.textAlign = 'left';
                xOffset = 0;
                // Draw justified line (except for last line)
                if (i < wrappedLines.length - 1) {
                    this.drawJustifiedLine(ctx, line, xOffset, yPos, textBoxWidth, layer);
                    return;
                }
            } else {
            ...
            if (layer.letterSpacing !== 0 || layer.kerning !== 0) {
                this.drawWithLetterSpacing(ctx, line, xOffset, yPos, layer, useInternalBW);
            } else {
                if (layer.strokeWidth > 0) {
                    ctx.drawingContext.strokeText(line, xOffset, yPos);
                }
                ctx.drawingContext.fillText(line, xOffset, yPos);
            }
        });
    ...
    wrapText(ctx, text, maxWidth, layer) {
        ...
        paragraphs.forEach(paragraph => {
            ...
            words.forEach(word => {
                ...
                const metrics = ctx.drawingContext.measureText(testLine);
    ...
    // Draw justified line (words spread across width)
    drawJustifiedLine(ctx, line, x, y, targetWidth, layer) {
        const words = line.split(' ');
        if (words.length <= 1) {
            // Single word, just draw normally
            if (layer.strokeWidth > 0) {
                ctx.drawingContext.strokeText(line, x, y);
            }
            ctx.drawingContext.fillText(line, x, y);
            return;
        }
        ...
        words.forEach(word => {
            totalWordWidth += ctx.drawingContext.measureText(word).width;
        });
        ...
        words.forEach((word, i) => {
            if (layer.strokeWidth > 0) {
                ctx.drawingContext.strokeText(word, currentX, y);
            }
            ctx.drawingContext.fillText(word, currentX, y);
            currentX += ctx.drawingContext.measureText(word).width + spaceWidth;
        });
    }
