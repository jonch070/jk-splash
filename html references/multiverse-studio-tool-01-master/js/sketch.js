...
function draw() {
    ...
    // When ONLY threshold is active (no dithering), force blur to apply to images
    // This prevents the compositing issues with threshold-only mode
    const forceBlurOnImages = thresholdActive && !ditheringActive;

    // Check if images should skip the blur effect
    // Only activate if: blur is enabled, applyBlurToImages is OFF, 
    // there are actual images, AND we're not forcing blur due to threshold-only mode
    const skipImagesFromBlur = PARAMS.showImageEffects &&
        PARAMS.imageEffects.blur > 0 &&
        !PARAMS.imageEffects.applyBlurToImages &&
        images.length > 0 &&
        !forceBlurOnImages;  // Don't skip if threshold-only mode
    ...
    // When skipImagesFromBlur, we need transparent background first, then composite images + bg later
    if (skipImagesFromBlur) {
        // Clear to transparent - images and background will be added via destination-over
        clear();
    } else if (useInternalBW) {
        background('#FFFFFF');
    } else {
        background(PARAMS.backgroundColor);
    }

    // Draw images at zIndex 0 (skip if they need to avoid blur)
    drawImagesByZIndex(0, skipImagesFromBlur);

    if (PARAMS.showGrid && modules.gridDistorter) {
        modules.gridDistorter.draw(useInternalBW);
    }

    // Draw images at zIndex 1 (skip if they need to avoid blur)
    drawImagesByZIndex(1, skipImagesFromBlur);

    // Apply effects BEFORE text if NOT applying to text
    if (PARAMS.showImageEffects && !PARAMS.imageEffects.applyToText) {
        applyImageEffects();

        // Draw skipped images AFTER blur using destination-over (draws UNDER existing content)
        if (skipImagesFromBlur) {
            drawingContext.globalCompositeOperation = 'destination-over';
            // Draw in reverse z-order with reverseOrder=true so newer images appear on top
            drawImagesByZIndex(3, false, true);
            drawImagesByZIndex(2, false, true);
            drawImagesByZIndex(1, false, true);
            drawImagesByZIndex(0, false, true);
            // Draw background under everything
            drawingContext.fillStyle = useInternalBW ? '#FFFFFF' : PARAMS.backgroundColor;
            drawingContext.fillRect(0, 0, width * pixelDensity(), height * pixelDensity());
            drawingContext.globalCompositeOperation = 'source-over';
        }
    }

    // Draw images at zIndex 2 (skip if they need to avoid blur)
    drawImagesByZIndex(2, skipImagesFromBlur);

    if (PARAMS.showTypography && modules.typographyMorpher) {
        modules.typographyMorpher.drawWithMask(useInternalBW);
    }

    // Draw images at zIndex 3 (skip if they need to avoid blur)
    drawImagesByZIndex(3, skipImagesFromBlur);
...
function applyPixelDithering() {
    ...
    const scale = Math.max(1, Math.round(dith.dots));
...
function drawTextBoundingBox() {
    ...
    // Get actual position (same as TypographyMorpher)
    let posX = layer.x;
    let posY = layer.y;
    ...
    // Split into lines (same as TypographyMorpher)
    const lines = textString.split('\n');
    const lineHeight = layer.fontSize * (layer.lineHeight || 1.2);
    const totalHeight = lines.length * lineHeight;

    // Find widest line (including letter spacing)
    let maxWidth = 0;
    lines.forEach(line => {
        let lineWidth = drawingContext.measureText(line).width;
        if (layer.letterSpacing && line.length > 1) {
            lineWidth += (line.length - 1) * layer.letterSpacing;
        }
    ...
    // Calculate box position - ANCHOR-BASED (matching TypographyMorpher)
    // Anchor point is at origin (0,0), text aligns to it
    let boxX;

    if (textAlign === 'left') {
        // LEFT: text starts at origin, extends right
        boxX = 0;
    } else if (textAlign === 'right') {
        // RIGHT: text ends at origin, extends left
        boxX = -textW;
    } else {
    ...
    // Vertical positioning - text is centered vertically around 0
    const boxY = -totalHeight / 2;

    // Accent color (same red as active buttons #FF164B)
    const accentColor = color(255, 22, 75);

    // Solid bounding box
    noFill();
    stroke(accentColor);
    strokeWeight(1.5);
    rect(boxX - padding, boxY - padding, textW + padding * 2, textH + padding * 2);

    // Anchor point at origin (0,0) - FIXED position
    fill(accentColor);
    noStroke();
    circle(0, 0, 12);
...
function drawImagesByZIndex(zIndex, skipImages = false, reverseOrder = false) {
    // If skipImages is true, don't draw anything (will be drawn later)
    if (skipImages) return;

    // When using destination-over, we need to draw in reverse order
    // so newer images (end of array) appear on top
    if (reverseOrder) {
...
function isMouseOverText(layer) {
    ...
    // Get position (same as TypographyMorpher)
    let posX = layer.x;
    let posY = layer.y;
    if (layer.useRelativePosition) {
        posX = width * (layer.relativeX || 0.5);
    ...
    // Transform mouse to local coordinates (inverse of layer transform)
    const rotation = radians(layer.rotation || 0);
    const cosR = cos(-rotation);
    const sinR = sin(-rotation);
    const localMx = (mx - posX) * cosR - (my - posY) * sinR;
    const localMy = (mx - posX) * sinR + (my - posY) * cosR;
    ...
    // Bounding box with padding - ANCHOR-BASED (matching renderer)
    const padding = 20;
    let boxX;

    if (textAlign === 'left') {
        // LEFT: text starts at origin
        boxX = -padding;
    } else if (textAlign === 'right') {
    ...
    // Check if local mouse is inside bounds
    return localMx >= boxX && localMx <= boxX + boxW &&
        localMy >= boxY && localMy <= boxY + boxH;
}
...
function touchStarted() {
    ...
    if (touches.length === 1) {
        // p5.js automatically sets mouseX/mouseY from touch
        mousePressed();
...
function updateUIForSelectedLayer() {
    ...
    if (layer && textInput) {
        ...
        const alignDots = document.querySelectorAll('.alignment-dot[data-align]');
        alignDots.forEach(dot => {
            if (dot.dataset.align === (layer.textAlign || layer.alignH || 'center')) {
...
function addTextLayer() {
    const newLayer = {
        text: 'NEW TEXT',
        fontSize: 140,          // SIZE: 140
        fontWeight: 900,        // WGHT: 900
        letterSpacing: -1,      // SPAC: -1
        lineHeight: 0.8,        // LINE: 0.8
        kerning: 0,
        x: 0,
        y: 0,
        ...
        rotation: 0,
        alignH: 'center',
        textAlign: 'center',      // Same as layer 0
        textBoxWidth: 80,         // Same as layer 0
        strokeWidth: 10,          // STRK: 10.0
        strokeColor: '#000000',
        fillColor: '#000000',
        uppercase: false,
        visible: true
...
document.addEventListener('click', (e) => {
    ...
    if (canvasContainer && canvasContainer.contains(target)) {
        return; // Let p5.js handle canvas clicks
    }

    // Don't deselect if clicking on any panel or interactive elements
    const isInteractive = target.closest('button') ||
        target.closest('input') ||
        target.closest('textarea') ||
        target.closest('.panel') ||              // Any panel
        target.closest('.panel-left') ||         // Left panel with text controls
        target.closest('.panel-middle') ||       // Middle panel with sliders
        target.closest('.panel-right') ||        // Right panel
        target.closest('.slider') ||
        target.closest('.vertical-slider') ||
        target.closest('.horizontal-slider') ||
        target.closest('.tab-btn') ||
        target.closest('.color-button') ||
        target.closest('.color-bar') ||
        target.closest('.alignment-dot') ||
        target.closest('.alignment-row') ||
        target.closest('.typography-controls') ||
        target.closest('.toggle-dot') ||
        target.closest('.apply-text-dot') ||
        target.closest('.apply-text-toggles-row') ||
        target.closest('.dithering-mode-dot') ||
        target.closest('.cell-shape-dot') ||
        target.closest('[data-h-slider-id]') ||
        target.closest('[data-v-slider-id]') ||
        target.closest('.width-slider-container') ||
        target.closest('.text-input-container');

    if (isInteractive) {
        return; // Don't deselect when clicking on controls
    }

    // Deselect all
    if (PARAMS.typography.selectedLayer !== -1 || selectedImageIndex !== -1) {
        PARAMS.typography.selectedLayer = -1;
        selectedImageIndex = -1;
        updateUIForSelectedLayer();
