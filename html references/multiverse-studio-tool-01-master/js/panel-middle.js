...
const tabVisibility = {
    grid: false,  // Synced with PARAMS.showGrid initial state
    secondo: true,
    terzo: true
};
...
function initPanelMiddle() {
    ...
    // Force UI update for active tab (buttons and specific containers)
...
function initEffectCheckbox() {
    ...
    // Apply blur to images toggle
    const blurImagesDot = document.getElementById('apply-blur-images-dot');
    if (blurImagesDot) {
        // Set initial state (default is off)
        if (PARAMS.imageEffects.applyBlurToImages) {
            blurImagesDot.classList.add('active');
        }

        blurImagesDot.addEventListener('click', () => {
            // Check if threshold-only mode (can't toggle in this mode)
            ...
            if (thresholdActive && !ditheringActive) {
                // In threshold-only mode, toggle is locked to ON
                return;
            }

            PARAMS.imageEffects.applyBlurToImages = !PARAMS.imageEffects.applyBlurToImages;

            if (PARAMS.imageEffects.applyBlurToImages) {
                blurImagesDot.classList.add('active');
            } else {
                blurImagesDot.classList.remove('active');
            }
...
// Update blur images toggle state based on threshold/dithering
function updateBlurImagesToggleState() {
    const blurImagesDot = document.getElementById('apply-blur-images-dot');
    if (!blurImagesDot) return;

    const thresholdActive = PARAMS.showImageEffects && PARAMS.imageEffects.thresholdEnabled;
    const ditheringActive = PARAMS.showTextEffects && PARAMS.dithering && PARAMS.dithering.enabled;

    if (thresholdActive && !ditheringActive) {
        // Force ON and disable appearance
        blurImagesDot.classList.add('active');
        blurImagesDot.classList.add('disabled');
        blurImagesDot.style.opacity = '0.5';
        blurImagesDot.style.cursor = 'not-allowed';
    } else {
        // Restore normal state
        blurImagesDot.classList.remove('disabled');
        blurImagesDot.style.opacity = '';
        blurImagesDot.style.cursor = '';
...
window.updateBlurImagesToggleState = updateBlurImagesToggleState;
...
function applyTabVisibility(tab, isVisible) {
    ...
    // Update blur images toggle state (locked when threshold-only mode)
    if (window.updateBlurImagesToggleState) {
        window.updateBlurImagesToggleState();
    }
...
function applyInitialSliderValues() {
    ...
    // DOTS (scale 1-10, always enabled)
    v = terzoValues[0] / 100;
...
function onHSliderChange(sliderIndex, value) {
    ...
    if (activeTab === 'grid') {
        ...
        // Dithering Effects
        switch (sliderIndex) {
            case 0: // DOTS (scale 1-10, always enabled)
                PARAMS.dithering.dots = Math.round(Utils.mapRange(v, 0, 1, 1, 10));
                PARAMS.dithering.enabled = true;
                break;
